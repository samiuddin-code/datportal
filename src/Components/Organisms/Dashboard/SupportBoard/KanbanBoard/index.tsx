...
import { techSupportColumnLabels } from "@helpers/commonEnums";
          ...
          <KanbanColumn
            ...
            <div className={styles.column}>
              <div className={styles.columnHead}>
                {techSupportColumnLabels[column as keyof typeof techSupportColumnLabels]} ({Number(meta[column - 1]?.total) ? Number(meta[column - 1]?.total) : ""})
              </div>
              <div>
                          ...
                          <div className={styles.kanbanItemBottom}>
                            <div style={{ display: 'flex', gap: 6, alignItems: 'flex-end' }}>
                              <div className={styles.cardBodyEnd}>
                                <div style={{ color: 'var(--color-light-300)', fontSize: 'var(--font-size-xs)' }}>
                            ...
                            <div className={styles.kanbanItemBottomBottom}>
                              ...
                              {item.AddedBy && (
                                <Tooltip
                                  overlayInnerStyle={{ borderRadius: '0.25rem', fontSize: 'var(--font-size-sm)' }}
                                  placement={item.status === 3 ? "left" : "bottom"}
                                  key={item.AddedBy?.uuid}
                                  title={"Added by: " + item.AddedBy?.firstName + " " + item.AddedBy?.lastName}>
                                  <Avatar
                                    size={25}
                                    style={{ border: '0.5px solid var(--color-light-200)' }}
