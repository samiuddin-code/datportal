...
import { EnquiryNotesType, EnquiryType } from '@modules/Enquiry/types';
...
const EnquiryNotes: FC<EnquiryNotesProps> = (props) => {
  const { enquiryId, onRefresh, permissions, open, onCancel } = props;
  const { loggedInUserData } = useSelector((state: RootState) => state.usersReducer);
  const { updateEnquiry, readEnquiry } = permissions;
  ...
  const logsCondition = useMemo(() => {
    return !!enquiryId && readEnquiry === true && activeTab === "logs"
  }, [enquiryId, readEnquiry, activeTab])

  // Fetch notes
  const {
    data: allNotes, onRefresh: onRefreshNotes, loading: notesLoading
  } = useConditionFetchData<EnquiryNotesType[]>({
    method: () => module.getNoteById(enquiryId),
    condition: !!enquiryId && readEnquiry === true,
  })
  ...
  const {
    ...
    method: () => module.getLogsById(enquiryId),
  ...
  const onAddNote = (note: string, isConcern: boolean) => {
    ...
    if (updateEnquiry === true) {
      ...
      module.addNote(enquiryId, { note, isConcern }).then((res) => {
