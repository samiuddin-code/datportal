...
import { BlogsResponseObject, BlogsTypes } from "../../../../Modules/Blogs/types";
...
interface BlogsModalProps extends PropTypes {
	record: number;
	permissions: { [key in BlogsPermissionsEnum]: boolean };
}

export const SiteBlogModal = (props: BlogsModalProps) => {
	...
	const blogsCategoryModule = useMemo(() => new BlogsCategoryModule(), [])

	const [recordData, setRecordData] = useState<Partial<BlogsResponseObject>>();
	const [countries, setCountries] = useState<CountryTypes[]>([]);
	const [blogsCategoryData, setBlogsCategoryData] = useState<BlogsTypes[]>([]);
	...
	const getBlogsCategorData = useCallback(() => {
		if (type !== "seo") {
			blogsCategoryModule.getAllRecords().then((res) => {
				if (res.data && res.data.data) {
					setBlogsCategoryData(res.data.data)
				}
			})
		}
	}, [blogsCategoryModule, type])
	...
	useEffect(() => {
		...
		getCountryList, getBlogsCategorData
	]);
	...
	return (
		<CustomModal
			...
			closable={true}
			onCancel={onCancel}
			titleText={type === "new" ? "Add New Blog" : type === "edit" ? "Edit Blog" : "SEO"}
			showFooter={false}
		>
			{recordData?.loading ? (
				<Skeletons items={10} />
							...
							<div>
								...
								>
									<CustomSelect
										...
										options={blogsCategoryData?.map((blogCat) => ({
											label: blogCat.localization[0].title,
											value: blogCat.id,
										}))}
										asterisk
									/>
