...
const RolePermissions = () => {
	...
	const handleRoleChange = () => {
		if (moduleId !== 0 && roles.selectedId !== 0) {
			permissionsModule
				...
				.catch((err) => {
					message.error(err.data?.message || "Something went wrong 3");
				});
		}
	};
	...
	const onFinish = () => {
		...
		if (permissionIds.length > 0) {
			permissionsModule.grantPrivilegesToRole({
				...
				message.error(err.data?.message || "Something went wrong 1");
			});
		}
		...
		if (deletedIds.length > 0) {
			permissionsModule.revokePrivilegesFromRole({
				...
				message.error(err.data?.message || "Something went wrong 2");
			});
		}
	};
	...
	return (
		<SiteSettingsTemplate className="pa-0">
			<div className="d-flex overflow-hidden">
				...
				<div className="d-flex flex-column w-100 px-4">
					...
					<div className="overflow-auto w-max-100 w-100">
						<div className={styles.content}>
							{permission.loading ? (
								...
								(moduleId === 0 || !moduleId) ?
									...
									:
									(
										<Form onFinish={onFinish} form={form} className="w-100 mb-10">
