...
import { BlogsResponseObject } from "../../../../Modules/Blogs/types";
...
interface BlogCategoryModalProps extends PropTypes {
	record: number;
	permissions: { [key in BlogsCategoryPermissionsEnum]: boolean };
}

export const SiteBlogCategoryModal = (props: BlogCategoryModalProps) => {
	const { openModal, onCancel, type, record,
		reloadTableData, permissions: { createBlogsCategory, updateBlogsCategory }
	} = props;
	...
	return (
		<CustomModal
			...
			closable={true}
			onCancel={onCancel}
			titleText={type === "new" ? "Add New Blog Category" : type === "edit" ? "Edit Blog Category" : "SEO"}
			showFooter={false}
		>
			{recordData?.loading ? (
				<Skeletons items={10} />
